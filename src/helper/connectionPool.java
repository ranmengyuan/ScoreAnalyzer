...
private String testTable = ""; // 测试连接是否可用的测试表名，默认没有测试表（测试创建的连接是否能够操作测试表）
...
//}
	...
	public  connectionPool() {
         this.jdbcDriver = "com.mysql.jdbc.Driver";// 数据库驱动
         this.dbUrl = "jdbc:mysql://127.0.0.1:3306/ScoreDeal?useUnicode=true&characterEncoding=UTF-8";// 数据库 URL
         this.dbUsername = "root";// 数据库用户名
         this.dbPassword = "B221gt12345";// 数据库用户密码
         try {
        	 createPool();
		} catch (Exception e) {
...
private Connection newConnection() throws SQLException {
         ...
         if (connections.size() == 0) {
          ...
          // 数据库返回的 driverMaxConnections 若为 0 ，表示此数据库没有最大  连接限制，或数据库的最大连接限制不知道  
...
public synchronized Connection getConnection() throws SQLException {
         ...
         while (conn == null){
          ...
          // 如果   getFreeConnection() 返回的为 null,则表明创建一批连接后也不可获得可用连接
...
* 当前没有可用的数据库连接，本函数则根据 incrementalConnections 设置的值创建几个数据库连接，并放入连接池中。
...
private boolean testConnection(Connection conn) {
         ...
         try {
          ...
          if (testTable.equals("")) {
           ...
           conn.setAutoCommit(true);   //将此连接的自动提交模式设置为给定状态
...
/**
* 刷新连接池中所有的连接对象,即分别使用一个新的连接代替旧的连接
*/
...
private void wait(int mSeconds) {
         ...
         try {
          ...
          Thread.sleep(mSeconds);  //调用线程停止指定的时间
